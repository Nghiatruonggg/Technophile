...
const CartBottom = ({ total, productList }) => {
  ...
  const orderInfo = {
    "customer_name": "Nghia",
    "status": "Pending",
    "item_numbers": productList.length,
  ...
  const handleCheckout = async () => {
    try {
      ...
      if (purchaseOrder.rejected.match(result)) {
        let errorMessage = result.payload.data?.error || "Please Try Again Later"
          ...
          <div className={styles.cartBottom}>
            <div className={styles.orderSummary}>
                    ...
                    <tr className="order-shipping">
                      ...
                      <td>
                        <div className={styles.inputGroup}>
                          <input
                            onChange={() => handleShippingClicked("Free Shipping")}
                            id="radio1"
                            type="radio"
                            name="shipping"
                            checked={defaultMethod == "Free Shipping" ? true : false}
                          />
              ...
              <div className={styles.checkoutButton}>
                <button type="submit" className={isLoading ? true : false}  onClick={handleCheckout}>Process To Checkout</button>
              </div>
            </div>
          </div>
        </Col>
